...
const ConsultationRecorder = ({ patientId }) => {
  ...
  const uploadConsultation = async (audioBlob) => {
    ...
    try {
      const res = await fetch("https://cliniq-copilot-dev.loca.lt/api/voice/consultation-record", {
        method: "POST",
        body: formData
      });
  ...
  const saveNote = async () => {
    setIsProcessing(true);
    try {
      const res = await fetch("https://cliniq-copilot-dev.loca.lt/api/intake/save", {
        method: "POST",
